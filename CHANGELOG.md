...
- **Mods de performance qui font le même travail.** La fiche d'un mod et la feuille « Conflits entre mods » signalent les paires actives qui patchent les mêmes méthodes du jeu (UltraSmooth, Radiance, Stardropium…), sans compter comme alerte.
...
- **Les boutons de la Quarantaine et les liens Nexus de la fiche restent lisibles en fenêtre étroite** : ils passent en icônes avec infobulle au lieu d'être coupés.
- **La page Diagnostic & Performances tient en fenêtre étroite.** Les filtres des journaux ne s'écrivent plus lettre par lettre, et les textes de la carte de santé et de la recherche du mod responsable ne sont plus coupés.
- **Toutes les rangées de boutons tiennent en fenêtre étroite** : recherche du mod responsable, éditeur de configuration, Découvrir, traduction installée sur la fiche passent sur deux lignes au besoin. L'onglet « Journal des modifications » de la fiche devient « Historique ».
...
- **Le badge « Mises à jour » ne compte plus une mise à jour déjà faite.** Le relevé SMAPI date du dernier lancement du jeu ; chaque entrée est confrontée à la version installée, et celle que le disque couvre déjà sort du compte comme de la page Mises à jour.
...
- **Le rapport de raccourcis voit ce qu'il ne voyait pas.** Les co-déclenchements au geste long (`F8` se tire aussi quand `Ctrl + F8` est tenu) ont leur section ; les collisions **manette** (`LeftStick` partagé par deux frameworks) ont leur catégorie, à part du clavier ; et les raccourcis d'un mod **en pause** ne sont plus invisibles — les collisions qui naîtraient à sa réactivation sont annoncées d'avance, hors du compte des problèmes avérés.
...
- **L'éditeur de configuration n'écrase plus une écriture faite par le mod lui-même.** Un mod réécrit son `config.json` quand il veut tant que le jeu tourne — au changement de profil, à une migration de version, à la fermeture d'un menu de réglages en jeu. L'éditeur lisait le fichier à l'ouverture et le remplaçait en bloc à l'enregistrement : ce que le mod avait écrit entre-temps disparaissait, et la sauvegarde du jour étant celle d'avant votre première édition, il n'en restait aucune copie. Le fichier est maintenant relu juste avant d'écrire, et une alerte vous laisse trancher. Un bandeau signale aussi la partie en cours, sur les mods actifs.
...
- **La vérification des mises à jour ne se déclare plus terminée alors qu'elle interroge encore Nexus** : quand smapi.io laisse des mods sans verdict, l'app reprend page par page sur Nexus — plusieurs dizaines d'appels, sur un quota limité là où smapi.io est gratuit. Cette reprise démarre au milieu du traitement de la première passe, et le relâchement de fin écrasait aussitôt son « en cours » : le bouton « Vérifier » réapparaissait, la barre de progression disparaissait, et un second passage complet pouvait démarrer par-dessus le premier. Le témoin reste maintenant allumé jusqu'à la dernière page.

- **Le hub de traductions thaï voyait la quasi-totalité de vos traductions comme absentes** : il cherchait `Mods/<nom>/i18n/th.json` alors qu'un mod en pause vit dans `Mods/.<nom>`. Sur votre parc, **22 des 30 traductions thaï installées** sont sous un mod en pause : elles s'affichaient « à installer », et le tri de la liste les reléguait en bas.
...
- **Une vérification des mises à jour interrompue en cours de route ne compte plus pour un tour complet** : vos mods partent chez smapi.io par lots de 150 — huit lots pour votre parc. Si l'un d'eux échoue (serveur momentanément indisponible, coupure réseau), les lots suivants ne partent pas ; l'app enregistrait pourtant un passage réussi, ce qui met la vérification automatique en pause pour **12 heures**. Jusqu'à 795 mods pouvaient ainsi rester une journée entière sans avoir été interrogés une seule fois, sans que rien ne le dise. Les lignes déjà connues étaient bien conservées — rien n'était perdu — mais la prochaine tentative n'arrivait pas. L'app sait maintenant qu'un tour est incomplet : elle le journalise et repart au lieu d'attendre.
...
- **Restaurer la configuration d'un mod en pause la rend enfin au mod** : la sauvegarde lit le dossier réel (un mod en pause porte un point), mais la restauration écrivait au nom sans point — dans un `Mods/MonMod` qu'elle créait au besoin, à côté du `Mods/.MonMod` bien réel. Ce dossier sans manifeste est invisible du jeu comme de l'app : la configuration n'était jamais restaurée, et l'app annonçait pourtant « sauvegarde restaurée ». La restauration suit maintenant le dossier tel qu'il est sur le disque, et un mod qui n'est plus installé est ignoré au lieu de voir son dossier fabriqué.

- **Un fichier impossible à écrire n'annule plus toute la restauration** : la restauration de configurations annonce ignorer un fichier manquant plutôt que de tout interrompre — mais un fichier qu'elle n'arrivait pas à *écrire* (verrouillé, appartenant à un autre compte) abandonnait bel et bien tous les mods restants, y compris ceux qui n'avaient aucun problème. Chaque fichier est désormais tenté pour lui-même.
...
- **Supprimer une sauvegarde d'installation ne laisse plus de coquille sur le disque** : chaque sauvegarde vit dans son propre dossier horodaté, et la suppression visait « le dossier parent » du mod sauvegardé. Pour un composant de pack, ce parent n'est que la coquille du pack : les fichiers partaient bien, mais le dossier horodaté restait, vide et retiré de l'index — donc invisible dans l'app et hors d'atteinte du ménage automatique. Votre magasin en compte **321**, pour 1 262 dossiers et 922 sauvegardes réellement listées ; **373 des sauvegardes restantes** en auraient produit une de plus à leur suppression. La règle est désormais la position dans `Backups/`, pas la profondeur du nom.

- **Le ménage automatique des sauvegardes ne bute plus sur un dossier en lecture seule** : une sauvegarde hérite des permissions du mod copié, et beaucoup de mods sont en lecture seule. La suppression manuelle rouvrait les droits avant d'effacer ; le ménage automatique, lui, avait perdu ce geste en route et abandonnait sans rien reprendre — à raison, il gardait alors la ligne d'index, si bien que ces sauvegardes étaient repoussées à chaque passage. Deux des vôtres sont dans ce cas. Deux autres suppressions — celles qui font marche arrière quand une sauvegarde échoue en cours de copie — avaient le même manque : elles laissaient derrière elles **19 dossiers** de votre magasin, quatre pour le seul *BushBloomMod* le même jour.

- **Éditer une sauvegarde ne la réécrit plus dans un autre encodage** : Stardew commence chacun de ses fichiers par une marque d'ordre des octets — les 38 fichiers écrits par le jeu sur votre disque en portent une. L'app, elle, la perdait : la fonction qui lit le fichier consomme cette marque, et celle qui l'écrit ne la remet pas. Trois copies de votre partie *Zofia* le montrent encore, longues de 37 492 144 octets contre 37 492 147 pour la version du jeu — exactement les trois octets manquants. Le jeu relit sans peine une sauvegarde sans marque, donc rien n'était cassé ; mais une partie vaut des centaines d'heures, et l'app la rend désormais comme elle l'a prise — sur les trois chemins qui la réécrivent : la fiche du joueur, l'inventaire, et la duplication d'une partie (qui touche aussi son fichier d'aperçu).
