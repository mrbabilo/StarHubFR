...
### Added
- **The mod list can be sorted by weight, and says what the current selection holds.** Sorting brings the heaviest folders to the top, and a figure in the toolbar totals whatever the filters currently keep — so scoping to paused mods and reading the total answers "how much can I get back" without adding up rows. On a real library that reads 12.7 GB.
- **Every row in the mod list now carries its weight.** Finding what takes up the room used to mean opening mods one at a time. The figure sits in the metadata strip, and turns orange past 100 MB — on a real library that marks 22 folders out of 863, and those 22 hold 87% of the 16.8 GB. A pack states the weight of its whole folder on its header row; its components show none, since that would count the same space once per component.
