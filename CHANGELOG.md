...
### Fixed
- **A profile no longer absorbs an activation that failed.** When a mod folder could not be moved — held open, permissions — the app wrote the resulting state back into the profile: the mod left enabled by the failure became a mod the profile *asked for*, and retrying had nothing left to do. The profile now keeps what you asked for.
