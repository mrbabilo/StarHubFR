...
### Changed
- **App renamed to StarHubFR at the OS level**: the app now identifies as **StarHubFR** to macOS — Dock, menu bar, Finder, and window title (`CFBundleName`/`CFBundleDisplayName`), matching the in-app branding. The bundle file (`StarHubTH.app`), executable, and bundle identifier stay `StarHubTH` internally.
