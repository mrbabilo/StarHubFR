...
### Changed
- **Build incrémental de l'app** (`F2-T2`, outil de développement). `python3 build_app.py --incremental` compile en objets par fichier avec le suivi de dépendances de swiftc, au lieu de recompiler les 211 fichiers en un seul module à chaque fois. Mesuré : 141,7 s → 2,4 s pour une modification isolée, 30,1 s dans le pire cas (changement de signature dans le ViewModel). Le binaire produit porte le même nombre de symboles définis (70 791) et la même architecture. Reste **opt-in** : le chemin par défaut ne bouge pas tant que l'app produite par ce chemin n'a pas été lancée.
...
### Fixed
- **Le bouton « Restaurer » d'une sauvegarde de secours n'affichait rien.** `SaveTimelineView` portait deux alertes construites avec l'API héritée de SwiftUI (celle qui rend un `Alert`) : dans ce cas une seule des deux se présente, et c'est celle de l'extérieur — la confirmation de restauration ne s'ouvrait donc jamais, pendant que celle de suppression fonctionnait. Les deux passent à l'API moderne (`alert(_:isPresented:actions:message:)`), qui s'empile correctement et que toutes les autres vues du dépôt utilisaient déjà. La sauvegarde ciblée est passée par `presenting:` plutôt que capturée : les actions s'exécutent après la fermeture, quand le binding a déjà remis la valeur à `nil`.
