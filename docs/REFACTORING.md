...
| 🛑 | **Constat d'arrêt — le ViewModel n'a plus de logique pure extractible** (mesuré le 2026-09-11, après la tranche 21) | Relevé sur le fichier lui-même, pas déduit : **337 fonctions**, dont **69 d'I/O**, **151 qui mutent l'état publié**, et 117 « calculs » qui sont en fait des **enveloppes de 1 à 13 lignes déléguant à Core**. Le plus gros calcul pur restant fait **13 lignes**. Les **128 `@Published`** (135 au relevé du §6, −7 depuis) sont désormais le sujet, et le §6 le disait déjà : *« les 135 propriétés publiées sont le vrai sujet »*.<br>**Conséquence pour la prochaine session** : chercher une tranche d'extraction de plus dans ce fichier produira des gains de quelques lignes — c'est ce que le relevé de rendement ci-dessous constate déjà sur les tranches 15 à 21. La suite était la **phase « vider le VM de son état publié »** (stores + P8) : elle a été cadrée, ouverte et **close depuis**.<br>✅ **Clause levée le 2026-09-14** — ce constat exigeait « un cadrage architectural dédié » et « l'accord explicite de l'auteur » avant d'ouvrir la suite. Les deux ont eu lieu : le cadrage est [`refactoring-vider-le-viewmodel.md`](refactoring-vider-le-viewmodel.md) §4, dont le bilan **clôt le chantier B le 2026-09-12** (les 8 domaines extraits pour leur état, en Core et testés), P8 est clos le 2026-09-13, et le découpage des vues a été livré le 2026-09-14 (T1–T4, bornes relevées sur les commits : `ModListView` 2 350→810 (`ac78f66d`), `ModDetailView` 2 144→1 396 (`70102805`), `MainView` 1 554→474 et `SavesView` 1 172→262 (`8ee2e4b3`)). Mesures du jour : `viewmodel_stored_state` **128 → 82 (2026-09-12) → 76**, `viewmodel_facades_to_combine` **0**, et les **5** commentaires `Façade provisoire` du ViewModel sont exactement les « cinq façades de lecture assumées (cond. 1, à demeure) » que le bilan désigne comme **l'état d'arrivée voulu**. Laisser la clause debout coûte une relecture complète à chaque passage : le **2026-09-14** elle a détourné la session de 17:15 vers un cadrage de substitution, et failli en faire écrire un second le soir.<br>⚠️ **La première moitié du constat, elle, tient** — et a été **reconfirmée par relevé indépendant le 2026-09-14**, sans relire ce verdict d'abord : 344 fonctions, **6 083 lignes de corps**, le plus gros corps **177** (`scanMods`, dont la logique pure vit déjà dans `ModScanner`). Les **trois zones jamais ouvertes** que le relevé de rendement ci-dessous désignait ont été passées au critère des entrées, et **aucune ne rend** : **B4-T4** délègue de bout en bout (`RecoverableFileScanner`, `TranslationRecoveryDiff`, `RecoveredFileWriter`, `ManifestJSON`, `I18nLenientParser`) — ce qui reste est de la plomberie d'I/O ; **Sauvegardes**, 22 fonctions dont le plus gros corps fait **38 lignes** ; **Liste & parc** et **tête n°2**, dont les gros corps (`toggleAllMods` 102, `deleteMod` 109, `performToggle` 112, `applyProfileToFilesystem` 149) délèguent tous leur **décision** à `ModFolderBulkMove`, `ModTrash`, `TogglePlan`, `ProfileApplyPlan`. **Ne pas rouvrir cette recherche** : elle a été faite deux fois |
| 22 🔶 | **Ancrage à l'installation — la boucle descend dans le store** — livrée le 2026-09-25 | `ModVersionAnchorStore.anchorInstalled(folderPaths:nexusFacts:now:)` (Core, **7 tests**, deux sabotages rouges : faits d'un pack gardés, `UniqueID` vide admis). `anchorInstalledMods` lisait les manifestes et posait les ancres **sans rien lire du ViewModel** — il en est la seule fonction de ce profil au relevé du jour (critère des entrées, §5). Le VM garde une façade d'une ligne, les deux vues appelantes n'ont pas bougé : −55 lignes. `affirmInstalled`, son voisin, reste au VM : il compose `refreshAffirmedUpdates`, le singleton Nexus et `republishUpdatesFromCache`. Dans la foulée, commentaires du VM compressés (3 804 → 1 593 lignes, `docs(vm)` ×10) : 9 851 → **7 622 lignes**, diff prouvé commentaires seuls ; trois docs orphelines rattachées à leur fonction. |

> ✅ **Périmé le 2026-09-14** : les deux domaines dits « non classés » ci-dessous **le sont**,
> et depuis le 2026-09-11 — Nexus aux lignes 5 à 9 du tableau (clos pour la logique),
> Traduction FR aux lignes 10 à 13 (close pour la logique). Seule la mise à jour entre
> parenthèses plus bas avait été faite, et pour Nexus seulement. Le paragraphe est gardé
> pour son raisonnement — pourquoi ces deux-là passaient en dernier — pas pour son état.
...
| --- | --- | --- |
| **Découper les vues** (leur P8, cible ~150 lignes) | **Au contact** : quand on extrait un domaine, on découpe la vue qui le consomme, dans le même mouvement | Au 2026-09-10 : `ModListView` **2340** lignes, `ModDetailView` **2145**, `MainView` **1536**, `SavesView` **1162**, `LogsView` 746 — la même pente que le ViewModel (`ModDetailView` a triplé depuis le relevé de 683). Une campagne dédiée serait un big-bang sans filet ; couplé à l'extraction, le découpage a une raison d'être et un périmètre.<br>🔶 **T1+T2 livrées le 2026-09-14** (cadrage dédié relu adversarialement, feu vert de l'auteur) : `ModListView` 2 350 → **806** (+ `ModListRow.swift` 1 141, `ModListView+Filters.swift` 416), `ModDetailView` 2 144 → **1 396** (+ `ModDetailSections.swift` 753) — déplacements purs, aucun body réécrit. ✅ Vérifié à l'écran par l'auteur le même jour (liste filtrée + tri, fiche à sections). 🔶 **T4 posée au cliquet le 2026-09-14** : `view_files_past_1500` — tout fichier de `Views/` qui dépasse 1 500 lignes doit se couper à son tour (périmètre `Views/` seul ; survit aux renommages, contrairement aux verrous `file:` indexés par chemin). 🔶 **T3 livrée le 2026-09-14** : `MainView` 1 554 → **474** (`UpdatesView` 600 rejoint le motif des autres destinations, sept satellites de la barre latérale → `SidebarComponents` 481), `SavesView` 1 172 → **262** (liste/grille/cartes → `SavesComponents` 353, éditeur + hero band → `SaveEditorView` 564) — déplacements purs, un seul élargissement d'accès (`farmDayLine` fileprivate → internal). `view_files_past_1500` resserré à **0** ; à ce jour plus aucune vue ne dépasse 1 500. ✅ **Les quatre tranches de P8 sont livrées** — reste le replay écran de l'auteur (barre latérale, mises à jour, parties/éditeur) |
...
| Dette | Déclencheur |
| --- | --- |
| `NSOpenPanel` appelé depuis le ViewModel (`:2321` dans `selectGameDir`, `:8728` dans `selectCustomAvatar` — coordonnées du 2026-09-10), ce qui rend ces fonctions intestables | ✅ **La moitié est faite** : le protocole `FilePicking` est né avec l'extraction d'Environnement (2026-09-10) et `selectGameDir` ne voit plus AppKit. ✅ **Close le 2026-09-14** : `selectCustomAvatar` passe par `ImagePicking` (second protocole, sa `LiveImagePicker` et son bouchon), et le ViewModel ne contient plus aucun `NSOpenPanel`. Sa règle de destination vit dans `CustomAvatarStaging` (Core, 5 tests, trois sabotages rouges). ⚠️ **Écart au plan annoncé ici** : la note prévoyait que `FilePicking` « gagnerait un `pickFile` ». C'est un **second protocole** qui est né — fondre les deux aurait donné à `GameEnvironmentStore` une méthode dont il n'a que faire, et obligé son bouchon à la porter |
...
| **Sauvegardes** | `reloadSaves`, `savesHierarchy`, `availableFilterTags`, le verrou d'écriture | 🔶 **Premier domaine du chantier B, extrait le 2026-09-11** — `SavesStore` (Core, `@Observable`, **11 tests**, quatre sabotages rouges) : la liste des parties, les trois préférences d'affichage, la hiérarchie filtrée et les étiquettes proposables. **Aucune vue touchée** — le suivi traverse la façade (cadrage §2, cas 1), ce que le chantier B confirme ici à l'usage et plus seulement au spike. L'étiquette arrive en **closure** (`tagForSave`) : le store ne connaît pas `SaveNotesStore`, le filtre est testable sans écrire dans le vrai domaine de préférences, et la lecture se faisant à l'appel le suivi traverse quand même. Le verrou d'écriture des huit fonctions devient un **test-et-pose en une opération** (`beginOperation()`/`endOperation()`), enfin sous test.<br>⚠️ **Le domaine n'est pas clos** : l'orchestration d'I/O reste au VM (`reloadSaves`, `editSave`, `saveInventory`, `deleteSave`, `duplicateSave`, les quatre backups) — ce sont des verbes, et ils traversent `showModal` et `localization`. Restent aussi cinq propriétés de **présentation**, dont `editingSave` et `viewingSaveTimeline` qui sont **inter-vues** (`MainView.swift:232`) et vont au store de navigation.<br>ViewModel : 10 331 → **10 312** ; `viewmodel_stored_state` 168 → **163**. ✅ **Condition 4 levée le 2026-09-11** : les six contrôles sont passés, dont le filtre par étiquette et la duplication — ce dernier étant le seul défaut du lot à la fois silencieux et collant (un relâchement de verrou manquant fige l'onglet jusqu'au prochain lancement). |
| **Journal** | `log(_:level:)`, `appendLogEntry`, `trimPreservingSignal`, `loadSmapiLog` | 🔶 **Tranche 1 extraite le 2026-09-11** — `LogBudget` (Core pur, **9 tests**, quatre sabotages) et `LogStore` (`@Observable`, **4 tests**). Le gain est la mise sous test de **deux bugs réels** que les commentaires décrivaient sans filet : un rechargement qui empilait une copie entière du journal SMAPI (N copies après N lancements), et un écrêtage par la **tête** du tableau combiné qui effaçait tout le journal de l'app dès que le journal SMAPI était gros. Le *quoi jeter* était déjà couvert (`LogNoise.trimIndices`) — seule la composition manquait.<br>🚩 **Le relevé du cadrage était faux sur deux points** : huit consommateurs et non deux (dont `BisectionRunner`, qui n'est pas une vue), et `logEntries` n'est pas un état « SMAPI » — 86 sites y écrivent des entrées `.app`. La couture réelle est l'arithmétique du plafond.<br>⚠️ **Une vue touchée**, exigée par le compilateur : `LogsView` mutait `vm.logEntries` par méthode (`removeAll`), ce qu'un relevé des `=` ne voit pas. « Vider les journaux » est descendu dans le store et **garde** le bloc SMAPI.<br>✅ **Tranche 2 livrée le 2026-09-11** — `SmapiHealthFold` (pur, **12 tests**, 5 sabotages) et `SmapiHealthStore` (`@Observable`, **8 tests**) : diagnostics, date, fraîcheur, alertes, conflits CP, verrou de relecture. Le store **garantit** ce que trois affectations voisines ne tenaient que par convention — date, diagnostics et conflits changent ensemble. Deux règles d'accumulation enfin sous test (diff des alertes par contenu et non par compte ; un journal sans date ou déjà replié n'est jamais replié deux fois). 🚩 `showSmapiAlerts` s'est révélée **morte** — une seule occurrence dans le dépôt, sa déclaration, née le 2026-07-03 : 5ᵉ instance de la famille « membre non câblé », trouvée parce que le relevé des écritures a rendu zéro.<br>✅ **Tranche 3 livrée le 2026-09-11** — `ErrorHistoryStore` (`@Observable`, **13 tests**, six sabotages) : l'historique par mod et par version, et ses trois mutations. Ce qu'elle achète n'est pas de la testabilité mais une **garde qui cesse d'être facultative** — tant que l'historique n'est pas lu du disque, rien ne doit le muter, sous peine d'écrire un historique vide par-dessus un fichier qui **ne se rebâtit pas**. Elle vivait en deux `if errorHistoryLoaded` posés au point d'appel.<br>🚩 `private lazy var` **ne compile pas** dans une classe `@Observable` (la macro en fait une calculée) : un crochet vers le propriétaire se pose après construction, patron de `NexusMetadataStore.setOnInvalidate`.<br>**Domaine clos pour son état** — ne reste que de l'orchestration, dont deux fonctions qui lisent `mods`. ViewModel : 10 331 → **10 266** sur les trois tranches ; `viewmodel_stored_state` 162 → **151**. ✅ Condition 4 des tranches 1 et 2 levée le 2026-09-11 ; celle de la tranche 3 due (historique sur la fiche d'un mod, renommage, corbeille).<br>🚩 **« Domaine clos » était prématuré — corrigé le 2026-09-12.** `outOfDateMods` (le bloc « You can update N mods » de SMAPI) restait au ViewModel, publié à part alors qu'il **sort de la même lecture** que la date et les diagnostics. Trouvé en reconnaissant le domaine 7 : son nom le faisait passer pour du Nexus, son producteur est `SmapiLogParser`. **Et c'était un défaut, pas seulement un reliquat** : `SmapiLogParser.updates(in:)` n'avait qu'un site d'appel, donc `parseAndAppendSmapiLog` (onglet Journaux, veilleur, bissection) republiait une date fraîche à côté de la liste du dernier scan — la pastille d'accueil compte `outOfDateMods + nexusUpdates`. Même trou que celui déjà bouché pour les conflits CP, dix lignes plus haut. Le paramètre `outOfDate` est **requis sans défaut** : la garde contre la récidive est le compilateur, pas un test (le VM n'est pas testable). 4 tests, quatre sabotages ; ViewModel inchangé à 10 288. ✅ **Condition 4 levée le 2026-09-12** : la pastille d'accueil suit bien le journal neuf après une partie, sans nouveau scan. | `parseAndAppendSmapiLog` ne bougera pas — elle lit `mods`, donc le domaine Scan, extrait en dernier.<br>ViewModel : 10 312 → **10 297** ; `viewmodel_stored_state` 163 → **162**. ✅ **Condition 4 levée le 2026-09-11** : les cinq contrôles sont passés, dont les deux qui rejouent les bugs historiques — un rechargement qui n'empile plus de copie du bloc SMAPI, et les lignes de StarHubFR qui survivent à un gros journal. |
| **Découverte** | `loadDiscovery`, `fetchDiscoverySection`, `searchDiscovery`, `loadDiscoveryDetail` | 🔶 **État extrait le 2026-09-11** — `DiscoveryStore` (`@Observable`, **8 tests**, cinq sabotages). Ce qu'il achète est un **compteur qui cesse d'exister en double** : `pendingSectionFetches` et son drapeau `discoveryLoading` étaient écrits deux fois verbatim. Trois règles enfin épinglées — le voyant s'éteint à la **dernière** réponse ; le plancher à zéro défend contre un rappel en trop ; « toutes catégories » (`nil`) est une identité, pas un joker.<br>Trois types entrent en Core avant le store : `NexusSearchError` (extrait de `NexusSearchClient`, alias conservé pour ses 8 appelants), `DiscoverySearchResult` et `DiscoveryDetailState`.<br>🚩 **Le cadrage annonçait 8 propriétés ; il y en a onze**, dont trois qui ne sont pas du domaine : les deux `RequestEpoch` (jetons d'orchestration) et `recentNexusInstalls` (de l'installation, lue par trois zones).<br>⚠️ Restent au VM : l'orchestration réseau, le cache `ModCatalog`, et `discoveryRows` qui lit `mods`.<br>ViewModel : 10 266 → **10 246** ; `viewmodel_stored_state` 151 → **143**. ✅ **Condition 4 levée le 2026-09-11** : les quatre contrôles sont passés, dont celui qui porte — le voyant de chargement s'éteint bien, c'est le compteur de requêtes en vol. |
| **Entretien & corbeille** | `buildMaintenanceReport`, `refreshTrash`, les actions de quarantaine | 🔶 **État extrait le 2026-09-11** — `MaintenanceStore` (`@Observable`, **7 tests**, trois sabotages) : l'inventaire, le dernier rapport de réparation, le message de quarantaine, les événements de corbeille, et le verrou de construction en test-et-pose. Deux distinctions épinglées — `report` a `nil` tant que l'inventaire n'est pas construit (distinct d'un rapport vide), et la corbeille **remplace** en bloc (relue du disque à chaque demande).<br>⚠️ **Deux façades `get`+`set`** : `QuarantineView` écrit l'état du domaine directement — la leçon `LogsView`, vue cette fois **avant** le gate par le relevé des écritures dans les vues.<br>⚠️ **Verrou de taille +6 lignes, assumé** : les façades `get`+`set` coûtent plus que les déclarations retirées ; les compteurs d'état baissent tous (143 → **138**). Le vrai remède, à la reprise des vues, est de donner à la vue un verbe nommé.<br>ViewModel : 10 246 → **10 252**. ✅ **Condition 4 levée le 2026-09-11** : les quatre contrôles sont passés, dont le verrou du bouton de construction et la bannière de réparation que rien n'efface à tort. |
| **Profils** | `saveProfiles`, `activateProfile`, `applyProfileToFilesystem`, `updateProfile` | 🔶 **État extrait le 2026-09-11** — `ProfileStore` (`@Observable`, **10 tests**, deux sabotages) : la liste, l'identifiant actif, le couple drapeau/identité de l'activation. Ce qu'il achète : le lookup `profile(with:)` (réécrit dix fois), `activeProfile` sans identifiant orphelin, et `endApplying()` qui efface **ensemble** drapeau et identité.<br>🚩 **Six mutations par indice manquées par le relevé** (`modProfiles[i].x = …`, `.merge`, `.setNote`) — troisième fois que le compilateur attrape ce que le relevé ne voit pas ; la règle pour la suite est de grepper tout ce qui méthode ou indice sur la propriété. Le store gagne `add`/`removeProfile`/`mutateProfile`.<br>⚠️ Verrou de taille **+17**, deuxième domaine de suite en hausse brute (les closures `inout`) — les compteurs d'état baissent (138 → **134**, 113 → **109**) ; la reprise des vues (P8) mérite une vraie tranche. `profileManagedConfigMods` reste au VM (magasin persisté, voisinage de `favoriteMods`).<br>ViewModel : 10 252 → **10 269**. ✅ **Condition 4 levée le 2026-09-11** : les cinq contrôles sont passés, dont le couple drapeau/identité qui disparaît ensemble et le verrou anti double activation. |
| **Traduction FR** | `searchTranslations`, `installTranslation`, `removeTranslation`, `linkToNexus` | 🔶 **Tranche 1 extraite le 2026-09-11** — `TranslationHubStore` (`@Observable`, **7 tests**, trois sabotages) : le registre des traductions posées, les deux moitiés de la recherche (propositions / posée — la moitié posée porte la pastille de mise à jour), les vols **mod par mod**. 14 mutations du registre passent par `mutateInstalled { … }`, blocs miroirs groupés ; la persistance reste à l'appelant.<br>🚩 **Le domaine était composite** — cinq familles là où le cadrage en voyait une : hub, couverture de profil (tranche 2), réglages IA/DeepL (**miroirs du chantier A**, restent), thaï (upstream, reste), hors domaine (`staleTranslationMods` au scan).<br>⚠️ Verrou de taille **+26**, troisième domaine de suite — **consigné : la reprise des vues (P8) doit devenir une tranche**, sinon la ligne brute du ViewModel ne descendra plus.<br>ViewModel : 10 269 → **10 295** ; `viewmodel_stored_state` 134 → **129**. ✅ **Condition 4 levée le 2026-09-11** : les cinq contrôles sont passés, dont celui qui porte — deux recherches successives n'héritent rien l'une de l'autre. Tranche 1 close.<br>🔶 **Tranche 2 (couverture de profil) extraite le 2026-09-12** — `ProfileTranslationStore` (`@Observable`, **7 tests**, trois sabotages) : les résumés par profil, la passe de mesure, le cache qui survit à la session. 🚩 **La `Task` n'était pas un verrou** — elle n'a jamais été annulée, on testait sa seule non-nullité ; le store porte un vrai test-et-pose, et le chemin « rien à mesurer » doit désormais **rendre** le verrou.<br>⚠️ **L'écriture disque reste à l'appelant** : `pruneCache` élague et rend ce qui reste — un store qui écrirait ne dirait pas *quand*. `summary(for:)` était sans appelant, le VM lui délègue.<br>**Première baisse de ligne brute depuis quatre domaines** : ViewModel 10 295 → **10 288** ; `viewmodel_stored_state` 129 → **123** ; `observable_stored_without_private_set` 109 → **105**. Aucune façade `get`+`set` ici — ce qui confirme le diagnostic, c'est l'écriture par la vue qui coûte, et P8 reste dû.<br>✅ **Condition 4 levée le 2026-09-12** : les quatre contrôles sont passés. **Domaine 6 clos** — les trois familles restantes (réglages IA/DeepL, thaï, hors domaine) ne sont pas des tranches dues, elles restent au ViewModel pour les raisons ci-dessus.<br>🔶 **Tranche 3 (les vols de la fiche) extraite le 2026-09-14** — `IdentitySearch`, `SupplementSearch` et leurs quatre états (`identitySearches`, `identitySearching`, `supplementSearches`, `supplementsSearching`) entrent dans `TranslationHubStore` (5 tests, prouvés rouges par sabotage sur quatre mécanismes). Les deux computed de `ModDetailView` lisent le store directement. 🚩 **Le cadrage « axe F » annonçait « les 9 hops de la recherche Nexus s'extraient bien » — la relecture l'a corrigé** : mesurés, les 9 sites font 271 lignes, et ce qui s'extrayait était leur **état**, pas leur orchestration. Le réseau reste au ViewModel (frontière documentée sur `DiscoveryStore`), les hops de la Découverte lisent `mods` via `discoveryRows`, ceux des traductions le trio UI. ViewModel : 10 084 → **10 045** ; `viewmodel_stored_state` 80 → **76**. |
| **Nexus** | `checkNexusUpdates`, `fetchNexusFallback`, `republishUpdatesFromCache`, `refreshAffirmedUpdates` | 🔶 **Tranche 1 (les mises à jour) extraite le 2026-09-12** — `ModUpdateStore` (`@Observable`, **11 tests**, huit sabotages) : les quatre listes (dues, en veille, affirmées, invérifiables), le voyant, l'erreur et la progression des deux passes.<br>🚩 **Le verrou avait deux détenteurs.** La reprise Nexus démarre *dans* la passe smapi.io : le relâchement de la première éteignait le voyant de la seconde, le bouton « Vérifier » réapparaissait pendant que Nexus était interrogé page par page, et un second passage complet pouvait démarrer aux dépens du quota. La défense était un `guard` à écrire au point d'appel ; c'est `endCheck()` qui refuse de relâcher — **une règle qui cesse d'être facultative**, comme `if errorHistoryLoaded` au domaine 2.<br>🚩 **28 propriétés relevées pour ~20 annoncées**, cinq familles (compte/clé, téléchargement, métadonnées suivent ; navigation et installation sont hors domaine).<br>⚠️ **Le store ne voit pas le cache plat** : consolidation par pack et fusion d'une passe partielle restent chez `NexusUpdateChecker.shared`. `updateSnoozer` reste au VM — c'est l'entrée de la partition, pas la partition.<br>**Deuxième baisse de ligne brute d'affilée** : ViewModel 10 288 → **10 270** (−18) ; `viewmodel_stored_state` 122 → **114** ; `observable_stored_without_private_set` 104 → **99**. Aucune façade `get`+`set` — le relevé des écritures par les vues les trouve **toutes** dans navigation et installation. Deux tuples deviennent des types (`SmapiVerdicts.Unverifiable` existait déjà ; `UpdateCheckProgress` rend la progression `Equatable`).<br>✅ **Condition 4 levée le 2026-09-12** : les quatre contrôles sont passés, dont celui qui porte — le voyant ne s'éteint pas entre la passe smapi.io et la reprise Nexus. (Ce contrôle est vide sans clé d'API : la reprise sort sur un `guard` avant de s'ouvrir ; la ligne « Reprise Nexus : N mods sans verdict » en est la preuve.)<br>🔶 **Tranche 2 (le compte et la clé) extraite le 2026-09-12** — `NexusAccountStore` (`@Observable`, **7 tests**, six sabotages). **Trois propriétés, cinq règles** : le compte appartient à la clé (une clé neuve invalide le compte connu) ; retirer la clé emporte compte et quota **et eux seuls** (les mises à jour ne lui doivent rien) ; **l'ignorance ne retire rien** — le sabotage qui compte est `isPremium != true`, la forme qu'on écrit sans y penser, qui cacherait le bouton à qui y a droit tant que Nexus n'a pas répondu ; la péremption à une semaine ; le lot du lancement.<br>⚠️ Le Trousseau n'entre pas dans le store : il ne retient que le **fait** que la clé a été acceptée. Une redondance de garde est laissée en place à dessein (`hasNexusApiKey || apiKey()`) — la retirer demanderait de prouver l'ordre du lot asynchrone.<br>`viewmodel_stored_state` 114 → **111** ; ViewModel inchangé à 10 270. ✅ **Condition 4 levée le 2026-09-12** : retirer la clé n'emporte pas les mises à jour déjà relevées.<br>🔶 **Tranche 3 (le téléchargement en vol) extraite le 2026-09-12** — `NexusDownloadStore` (`@Observable`, **8 tests**, cinq sabotages). **La moitié de la famille n'était pas de l'état** : quatre des huit propriétés sont des collaborateurs (tâche annulable, file, estimateur de débit, magasin d'archives), tous déjà en Core. L'asymétrie corrigée : la remise au repos était regroupée, l'**ouverture** restait posée à la main sur les deux seuls sites de départ (deux, vérifié par grep global). Règles épinglées : la barre reste muette pendant que le lien se résout ; **annuler ne conclut pas** (`URLSession` conclut par le chemin d'échec) ; le débit ne survit pas à son transfert.<br>🚩 **Deux tests étaient creux**, trouvés par sabotage : le débit (deux relevés dans la même milliseconde n'en rendent aucun — `now` est désormais injecté) et la barre remise à zéro à l'ouverture (un store neuf l'a déjà à `nil` — il fallait un second transfert). Troisième fois en une session ; point commun : **l'état initial satisfait déjà l'attente**.<br>🚩 **Le cliquet surcomptait de 44 %**, trouvé en passant : 29 des 95 violations de `observable_stored_without_private_set` étaient des `private var`, plus strictes que `private(set)` ; et le motif ne voyait pas `public private(set) var`, l'ordre idiomatique Swift. Base 95 → **66**, outil reprouvé capable d'échouer (`65aabd6`).<br>ViewModel 10 270 → **10 254** (−16, troisième baisse d'affilée) ; `viewmodel_stored_state` 111 → **105**. ✅ **Condition 4 levée le 2026-09-12** : annuler ne remet pas l'état au repos avant que l'annulation ne soit rapportée.<br>✅ **Domaine clos à trois tranches le 2026-09-12 — pas de tranche 4.** Les quatre propriétés restantes sont examinées et laissées au VM, raisons au cadrage §4 : `nexusCategories`/`nexusModExtras` sont des miroirs d'un cache dont le checker garde la persistance **et** le verrou (un store n'y épinglerait rien — l'inverse de la tranche 2) ; `modDetailState` a ses transitions dans son propre type ; les archives ont déjà leur magasin Core. Seul resserrage : `nexusArchiveStore` `var` → **`let`**.<br>🚩 **Une hypothèse de défaut vérifiée puis abandonnée** : `fetchMetadata` n'a pas l'air de persister, mais `fetchSingleMod` qu'elle appelle le fait déjà, sous verrou et avec garde de génération. Lire le chemin du VM sans lire la fonction appelée faisait voir un trou inexistant.<br>Ne reste au VM que de l'orchestration réseau. |
...
| Avatar de sauvegarde, `CustomAvatarStaging` (2026-09-14) | Une destination **déjà occupée est retirée** avant la copie, là où `selectCustomAvatar` appelait `copyItem` sur une place prise | **Correction, pas simple déplacement.** Deux images distinctes peuvent porter le même nom de fichier ; la destination composée est alors identique et `copyItem` refuse d'écrire — `NSFileWriteFileExists` (516), **mesuré en exécutant le cas**, l'ancienne image restant en place. Le joueur voyait son choix rester sans effet, et seule une ligne de journal le disait. Le retrait est sûr parce que le préfixe de destination porte `folderName`, qui **est** l'identité d'une sauvegarde (`SaveInfo.id` le rend tel quel) : le fichier écarté ne peut appartenir qu'à elle. Testé |
| Avatar de sauvegarde, `ImagePicking` (2026-09-14) | Un **second** protocole de sélection, au lieu d'une méthode ajoutée à `FilePicking` | `FilePicking` choisit un dossier, `ImagePicking` un fichier image : les fondre donnerait à `GameEnvironmentStore` une méthode dont il n'a que faire, et obligerait son bouchon à la porter. C'est la forme que le §3 prescrit — un protocole par frontière d'I/O. Ce commit sort le **dernier** `NSOpenPanel` du ViewModel, refermant la dette de couche ouverte au 2026-09-10 |
