...
| 🚩 | **Un N+1ᵉ chemin trouvé, non corrigé** — à trancher | Le report de renommage de clés (C2-T4, `applyRenameReportTranslation`, VM `:6801`) compose `i18n/fr.json` **en dur** : sur un mod en layout B, `Data(contentsOf:)` échoue et la boucle fait `continue` — **sans journaliser**. Les **5 mods du parc** relevés ci-dessus ne reçoivent donc aucun report de renommage, en silence. Le remède est `I18nLocaleResolver.files(in:locale:)`, déjà en Core et déjà testé ; il change un comportement observable (des clés renommées sur des mods qui n'en recevaient pas) et mérite son propre commit en TDD, pas une correction glissée dans une extraction |
