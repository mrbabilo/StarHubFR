...
| **Détail de mod** | `loadModDetail`, `fetchModDetailRemote`, `markDetailNotLoading` | 🔶 **Tranche 1 extraite le 2026-09-10** — `ModDetailState` (Core, 5 tests) : les transitions cache instantané → repli local → rafraîchissement, et l'arrêt du spinner propre au mod affiché. Le câblage reste au VM — `didSet` d'ouverture, garde anti-course à l'application (`viewingModDetail` peut avoir changé pendant le fetch), sauvegarde du cache — et le réseau vit déjà dans `NexusUpdateChecker` (Core). Ce qui reste relève du domaine Nexus, non classé |
