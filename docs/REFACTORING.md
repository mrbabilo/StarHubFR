...
| **Détail de mod** | `loadModDetail`, `fetchModDetailRemote`, `markDetailNotLoading` | 🔶 **Tranches 1 et 2 extraites le 2026-09-10** — `ModDetailState` (Core, 5 tests) : les transitions cache instantané → repli local → rafraîchissement, et l'arrêt du spinner propre au mod affiché ; `NexusModIdentity` (Core, 4 tests) : l'identifiant effectif (override utilisateur > manifeste), l'id résolu d'un pack (premier enfant qui a quelque chose), le lien public, l'extra. Le câblage reste au VM — `didSet` d'ouverture, garde anti-course à l'application, sauvegarde du cache — sous forme de façades provisoires (§6, cond. 1), et le réseau vit déjà dans `NexusUpdateChecker` (Core). Ce qui reste relève du domaine Nexus, non classé |
