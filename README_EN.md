...
*   **Advanced Mod List**: Automatic classification by **type** (UI, Framework, Content Patcher, Translation, NPC, Audio, Map…) inferred from the manifest, also used as an offline fallback for the category filter. Five scopes up top — All, Enabled, Paused, **Issues**, **Updates** — each with its count. Category filtering, pagination (12 mods/page with direct page jump), uncategorized mod filter, "With Config" filter (configurable mods only), sorting by name (A-Z/Z-A), author, version or activation order. A gear icon on each configurable mod opens the config editor directly.
*   **Save Manager**:
