...
final class LaunchSplashController {
    ...
    func show(vm: StarHubTHViewModel) {
        ...
        // Survives Spaces switching and doesn't clutter Mission Control with a
        // borderless frame the user can't act on.
    ...
    private func mainAppWindow() -> NSWindow? {
        NSApp.windows.first { $0.identifier?.rawValue.contains("main") == true }
            ?? NSApp.windows.first { $0 !== panel && $0.contentViewController != nil }
    }
